...
switch (_action) do {
	case "onLoad": {
		...
		((uiNamespace getVariable "cti_dialog_ui_tabletmenu") displayCtrl 777101) ctrlEnable (if ((CTI_Base_ControlCenterInRange || CTI_Base_GearInRange_Depot) && !CTI_P_PreBuilding) then {true} else {false}); //gear available everywhere. if cc destroyed then cannot access equipment unless near depot or barracks
		//--- Factory available?
