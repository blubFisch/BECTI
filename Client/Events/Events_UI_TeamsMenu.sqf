	...
	case "onTransferAllPressed": {
		...
		if (_amount > -1) then {
			...
			if (_funds >= (_amount * count(uiNamespace getVariable "cti_dialog_ui_teamsmenu_groups"))) then {
				{[_x, _amount] call CTI_CO_FNC_ChangeFunds} forEach (uiNamespace getVariable "cti_dialog_ui_teamsmenu_groups");
