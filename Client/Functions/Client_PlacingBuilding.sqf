...
while {!CTI_VAR_StructurePlaced && !CTI_VAR_StructureCanceled} do {
	...
	if (time - _last_collision_update > 2) then {_last_collision_update = time;{_local disableCollisionWith _x} forEach (player nearEntities 150)};
	CTI_P_PreBuilding_SafePlace = if (!surfaceIsWater _pos && !(lineIntersects [ATLtoASL (player modelToWorld (player selectionPosition "pilot")),ATLtoASL (_local modelToWorld (_local selectionPosition "pilot")), player, _local]) && count((nearestObjects [getPos _local, ["Building"], 12]) - [_local]) < 1) then {true} else {false};
