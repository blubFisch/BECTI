...
_restrict=[];
if (CTI_SM_NONV isEqualTo 1) then {
	//No NV
...
if (CTI_SM_NONV isEqualTo 2) then {
	//No Thermals
...
if (CTI_SM_NONV isEqualTo 3) then {
...
if (CTI_isClient) then {
	...
	if (side player isEqualTo resistance) exitWith {false};
	...
	missionNamespace setVariable ["cti_gear_all",_gear];
 	_func1={
		...
		while {!CTI_GameOver} do
		{
			{
				_i=_x;
				if ({_i isEqualTo _x} count _restrict >0) then {
