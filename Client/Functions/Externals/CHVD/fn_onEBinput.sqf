...
_textValue = if (_textValue == "") then {1} else {call compile _textValue min 12000 max 1};
...
switch (_varType1) do {
	...
	case 'CHVD_car': {
		_viewDistValue = _textValue min CHVD_maxViewVeh;
	};
	case 'CHVD_air': {
		_viewDistValue = _textValue min CHVD_maxViewAir;
	};
};
