...
_block_key = (findDisplay 46) displayAddEventHandler ["KeyDown","if ((_this select 1) isEqualTo 45) then {true} else {false}"];// disable key X 
...
actionId = _spieler addAction ["<t color=""#FFAD1F"" size='2' shadow='2'>" +"Drop", "Client\Functions\Externals\BDD\loslassen.sqf", [], 99, true, true, "", "(attached && (_target isEqualTo _this))"];
