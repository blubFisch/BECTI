...
onMapSingleClick '
	...
	_large_fob = [ATM_Jump_clickpos, _large_fobs] call CTI_CO_FNC_GetClosestEntity;
	...
	if (_upgrade_halo > 1) then {
		if ((ATM_Jump_clickpos distance  _ct < CTI_MARKERS_TOWN_AREA_RANGE) || (ATM_Jump_clickpos distance  _fob < CTI_MARKERS_TOWN_AREA_RANGE) || (ATM_Jump_clickpos distance  _large_fob < CTI_MARKERS_TOWN_AREA_RANGE) || (ATM_Jump_clickpos distance  _base < CTI_BASE_AREA_RANGE)) then { 
