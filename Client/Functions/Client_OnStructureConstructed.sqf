...
if (CTI_Log_Level >= CTI_Log_Information) then {
	["INFORMATION", "FILE: Client\Functions\Client_OnStructureConstructed.sqf", format["Base Structure [%1 (%2)] construction is now available at position [%3]", _structure, (_var select CTI_STRUCTURE_LABELS) select 1, getPos _structure]] call CTI_CO_FNC_Log;
};

//--- Add score for the commander
if (call CTI_CL_FNC_IsPlayerCommander) then {
	_score = round((_var select CTI_STRUCTURE_PRICE) / CTI_SCORE_BUILD_VALUE_PERPOINT);
	
	if (CTI_Log_Level >= CTI_Log_Debug) then {
		["DEBUG", "FILE: Client\Functions\Client_OnStructureConstructed.sqf", format ["Base Structure [%1 (%2)] construction did reward the player's with a score bonus of [%3]", _structure, (_var select CTI_STRUCTURE_LABELS) select 1, _score]] call CTI_CO_FNC_Log;
	};
