...
/*
  ...
  # PARAMETERS #
    ...
    5	{Optionnal} [Number]: The amount of scans to perform
  ...
  # SYNTAX #
	...
	[POSITION, RADIUS MIN, RADIUS MAX, DISTANCE MIN, TEMPLATE, PASSES] call CTI_CO_FNC_GetSafePosition
	
  # EXAMPLE #
    _empty_position = [vehicle player] call CTI_CO_FNC_GetSafePosition
    _empty_position = [vehicle player, 1, 50, 10, "vehicles", 250] call CTI_CO_FNC_GetSafePosition
*/

params["_center", ["_radius_min", 1], ["_radius_max", 200], ["_distance_min", 15], ["_template", ""], ["_passes", 500]];
private ["_position"];
...
for '_i' from 1 to _passes do {
	...
	if (count (nearestTerrainObjects [_center_ran, _filter, _distance_min, false]) < 1 && !surfaceIsWater _center_ran) exitWith {_position = _center_ran};
};
