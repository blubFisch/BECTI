...
switch (_script) do {
	case "salvager": {if (CTI_IsServer) then {(_vehicle) execFSM "Server\FSM\update_salvager.fsm"} else {["salvager", _vehicle] remoteExec ["CTI_PVF_SRV_RequestHandleSpecialVehicle", CTI_PV_SERVER]}};
	case "salvager-independent": { _sideLogic = (_side) call CTI_CO_FNC_GetSideLogic; _sideLogic setVariable ["cti_salvagers", ((_sideLogic getVariable ["cti_salvagers", []]) pushBack _vehicle), true]; [_side, _vehicle] execFSM "Server\FSM\update_salvager_independent.fsm"};
