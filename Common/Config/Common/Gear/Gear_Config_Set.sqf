		...
		if (isNil {missionNamespace getVariable _item}) then {
			switch (_config_type) do { //--- Each item is handled differently
				case "CfgGlasses": {
					...
					if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Set Glasses [%1].", _item]] call CTI_CO_FNC_Log };
				};
				case "CfgMagazines": {
					...
					if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Set Magazine [%1].", _item]] call CTI_CO_FNC_Log };
				};
				case "CfgVehicles": {
					if (getNumber(configFile >> _config_type >> _item >> 'isbackpack') == 1) then { //--- Make sure that this is a backpack
						...
						if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Set Backpack [%1].", _item]] call CTI_CO_FNC_Log };
					};
				};
				case "CfgWeapons": {
					...
					if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Set Item [%1] of nature [%2].", _item, _item_type]] call CTI_CO_FNC_Log };
				};
			};
		} else {
			if (CTI_Log_Level >= CTI_Log_Warning) then { ["WARNING", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Item [%1] was skipped since it is already defined.", _item]] call CTI_CO_FNC_Log };
		};
	} else {
		if (CTI_Log_Level >= CTI_Log_Error) then { ["ERROR", "FILE: Common\Config\Gear\Common\Gear_Config_Set.sqf", format ["Item [%1] does not belong to any known classnames. If it belong to an Addons, make sure that it is loaded.", _item]] call CTI_CO_FNC_Log };
	};
};

if (CTI_Log_Level >= CTI_Log_Information) then { ["INFORMATION", "FILE: Common\Config\Common\Gear\Gear_Config_Set.sqf", format ["Initialized Gear [%1], set [%2/%3]", _faction, _cpt, count _items]] call CTI_CO_FNC_Log };
