...
for '_i' from 0 to (count _c)-1 do {
	...
	if (isClass (configFile >> "CfgVehicles" >> _classname)) then {
		...
		if (isNil "_get") then {
			...
			_stored = [_label, _picture, _o select _i, _t select _i, _u select _i, _f select _i, _turrets, _scripts, _d select _i, _g select _i];
			missionNamespace setVariable [_var_name, _stored];
			_cpt = _cpt + 1;
			
			if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Common\Units\Set_Units.sqf", format ["[%1] Set unit [%2] using classname [%3]", _side, _var_name, _classname]] call CTI_CO_FNC_Log };
		} else {
			if (CTI_Log_Level >= CTI_Log_Warning) then { ["WARNING", "FILE: Common\Config\Common\Units\Set_Units.sqf", format ["[%1] unit [%2] was skipped since it is already defined", _side, _var_name]] call CTI_CO_FNC_Log };
		};
	} else {
		if (CTI_Log_Level >= CTI_Log_Error) then { ["ERROR", "FILE: Common\Config\Common\Units\Set_Units.sqf", format ["[%1] unit [%2] is not using a valid CfgVehicles classname [%3]. If it belong to an Addons, make sure that it is loaded.", _side, _var_name, _classname]] call CTI_CO_FNC_Log };
	};
};

if (CTI_Log_Level >= CTI_Log_Information) then { ["INFORMATION", "FILE: Common\Config\Common\Units\Set_Units.sqf", format ["[%1] [%2/%3] units were defined", _side, _cpt, count _c]] call CTI_CO_FNC_Log };