...
for '_i' from 0 to (count _headers) -1 do {
	...
	switch (typeName _header) do {
		case "STRING": { _classnamevar = _classname; };
		case "ARRAY": {
			_classnamevar = _classname;
			if ("Armed" in ((_header select 1) select 0)) then {
				_classname_variant = ((_header select 1) select 0) select 1;
				_classnamevar = format["%1_%2",_classname,_classname_variant];
			};
	...
	if (isNil {missionNamespace getVariable format["CTI_%1_%2",_side,_classnamevar]}) then {
		...
		missionNamespace setVariable [format["CTI_COIN_%1_DEFENSE_CATEGORY_%2", _side, _find], (missionNamespace getVariable [format["CTI_COIN_%1_DEFENSE_CATEGORY_%2", _side, _find], []]) + [format["CTI_%1_%2",_side,_classnamevar]]];
		...
		if (CTI_Log_Level >= CTI_Log_Debug) then { ["DEBUG", "FILE: Common\Config\Base\Set_Defenses.sqf", format ["[%1] Set Defense [%2] using variable name [%3]", _side, _classnamevar, format["CTI_%1_%2", _side, _classnamevar]]] call CTI_CO_FNC_Log };
