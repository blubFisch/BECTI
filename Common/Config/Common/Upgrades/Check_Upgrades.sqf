...
_values = [];
for '_i' from 0 to count(_levels)-1 do {_values pushBack []};

{
	_upgrade = _x select 0;
	_level = _x select 1;
	if (_enabled select _upgrade) then {
		if !(_level in (_values select _upgrade)) then {_values set [_upgrade, (_values select _upgrade) pushBack _level]};
	};
} forEach _orders;
...
if (count _add > 0) then {
	...
	if (CTI_Log_Level >= CTI_Log_Information) then { ["INFORMATION", "FILE: Common\Config\Common\Upgrades\Check_Upgrades.sqf", format ["[%1] AI Commander upgrade order has been completed with [%2]", _side, _add]] call CTI_CO_FNC_Log };
};