...
private ["_logic", "_side", "_var", "_virtual_damages", "_ruins", "_fobtype", "_health", "_lastdamagetime", "_lastdamagediff"];
...
if (_virtual_damages >= 1 || !(alive _damaged)) then {
	...
	if (CTI_Log_Level >= CTI_Log_Information) then {
		["INFORMATION", "FILE: Server\Functions\Server_OnFOBHandleVirtualDamage.sqf", format["A [%1] structure from side [%2] has been destroyed (virtual damages) by [%3]", "FOB", _side, _shooter]] call CTI_CO_FNC_Log;
	};
