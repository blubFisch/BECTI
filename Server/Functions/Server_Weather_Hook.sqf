...
if (CTI_WEATHER_MONSOON_COEF isEqualTo 0) then {_monsoon_variance_coef_setting = 1};
...
if (CTI_WEATHER_STORM_TIME isEqualTo 0) then {_storm_time_setting = 120};
if (CTI_WEATHER_STORM_TIME > 0) then {_storm_time_setting = CTI_WEATHER_STORM_TIME};
if (CTI_WEATHER_STORM_TIME isEqualTo -1) then { _storm_time_setting = random 1800 };
	...
	*/
	...
	if (CTI_WEATHER_VARIANCE_TIME isEqualTo 0) then {
		...
		while {true} do {
			...
			if (CTI_WEATHER_RAIN isEqualTo -1) then { _rain_setting = random [0,0.2,0.5] };
			if (CTI_WEATHER_RAIN_COEF isEqualTo -1) then {_rain_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_OVERCAST isEqualTo -1) then { _overcast_setting = random 1 };
			if (CTI_WEATHER_OVERCAST_COEF isEqualTo -1) then {_overcast_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_FOG isEqualTo -1) then { _fog_setting = random [0,0.05,0.3] };
			if (CTI_WEATHER_FOG_COEF isEqualTo -1) then {_fog_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_FOG_DECAY isEqualTo -1) then { _fog_decay_setting = random 1 };
			if (CTI_WEATHER_FOG_DECAY_COEF isEqualTo -1) then {_fog_decay_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_FOG_ALT isEqualTo -1) then { _fog_alt_setting = random 1 };
			if (CTI_WEATHER_FOG_ALT_COEF isEqualTo -1) then {_fog_alt_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_WIND isEqualTo -1) then { _wind_setting = random 1 };
			if (CTI_WEATHER_WIND_COEF isEqualTo -1) then {_wind_variance_coef_setting = random (0.99)};
			if (CTI_WEATHER_WAVES isEqualTo -1) then { _waves_setting = random 1 };
			if (CTI_WEATHER_WAVES_COEF isEqualTo -1) then {_waves_variance_coef_setting = random (0.99)};
...
while {!CTI_GameOver} do {
	...
	_nexttime setFog [_nextfog,0.01 + random (0.04),random(10)];
	_nextover=random 1;
	if (CTI_WEATHER_ALLOWRAIN isEqualTo 0) then {_nexttime setRain 0} else {_nexttime setRain random(_nextover)};
	_nexttime setOvercast _nextover;
	_fog_forecast=fogForecast;
    _overcast_forecast=overcastForecast;
}*/